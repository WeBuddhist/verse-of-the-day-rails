---
ref: sa-1
source_ref: "SĀ 1"
canon: Chinese Āgama
unit_type: single
source_text: 1-SOURCES/Text/zh-samyukta-agama.md
source_block: "^sa1-0"
grounding: translation
translations: [patton]
theme: impermanence
concepts: [無常 (impermanence), 厭離 (disenchantment), 心解脫 (liberation of mind)]
status: draft
---

## Source Text

![[1-SOURCES/Text/zh-samyukta-agama.md#^sa1-0]]

> Full Literary-Chinese sūtra is in the source file under this heading
> (paragraph blocks `^pT02p0001a…`). It is short — the canonical SĀ opening on
> contemplating the five aggregates as impermanent.

## Authoritative Renderings

### Charles Patton — English (CC0, SuttaCentral)

![[1-SOURCES/Translations/en-samyukta-agama-patton.md#^sa1]]

## Traditional Interpretation (commentary)

*No commentary imported* (no clean classical Āgama verse-commentary exists; see vault-annex §7).

## Disambiguated Meaning

Grounded in the Chinese source and Patton's translation (1-SOURCES/Translations/en-samyukta-agama-patton.md#^sa1):

The Buddha, at Sāvatthī, tells the monks: contemplate form (*rūpa* / 色) as impermanent (無常). To see it so is right contemplation; right contemplation gives rise to disenchantment (厭離); with disenchantment, delight-and-greed (喜貪) is exhausted; the exhaustion of delight-and-greed is called liberation of the mind (心解脫). The same is to be contemplated for feeling, perception, formations, and consciousness — and likewise as suffering, empty, and not-self.

- This is a short prose discourse, not a verse. For verse of the day, the usable pith is the chain: *impermanent → disenchantment → fading of greed → liberation of mind.* A rendering should keep that causal sequence intact.
- 色/受/想/行/識 = the five aggregates; render in each language's standard Buddhist vocabulary (flag for termbase).

## Theme & Selection Notes

Theme: **impermanence / the five aggregates.** The opening sūtra of the Saṁyukta Āgama; foundational and concise. Demonstrates the Chinese-canon rail. As prose, pick a single line for the daily card (e.g. the impermanence→liberation chain) rather than the whole discourse.

## Concept Links

*(populated once Local-Wiki articles exist)*
- 無常 (impermanence)
- 厭離 (disenchantment)
- 心解脫 (liberation of mind)
