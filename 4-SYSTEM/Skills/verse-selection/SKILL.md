...
- **Corpus** — `1-SOURCES/Text/` (+ paired translations) to pick from. **Draw from the whole corpus, not just the verse collections.** Source pools:
  ...
  - *Chinese — beyond T210:* the four **Āgamas** (DĀ/MĀ/SĀ/EĀ) for more Chinese prose sentences (quote a self-contained sentence where one exists). **Do NOT keep pulling only T210** (the Chinese Dharmapada). Note: Chinese *Mahāyāna* sūtras are not in the vault — Mahāyāna representation comes through the **Tibetan Kangyur** pool above; if Chinese-language Mahāyāna is wanted later, import from CBETA (e.g. Vimalakīrti T475, Lotus T262).
