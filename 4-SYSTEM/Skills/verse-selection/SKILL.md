...
4. **Pick a candidate — check the candidate pool first.** Open the target canon's pool file (`candidate-pool/pali.md` / `chinese.md` / `tibetan.md`) and filter to `status: unclaimed`; among those, prefer an entry whose `theme` fills a **Known gap** in `theme-checklist.md`. Pool entries were already gate-, buddhavacana-, and verbatim-checked by `candidate-tagging`, so you confirm they still satisfy (a)–(d) below but don't re-derive them from raw source. **Only fall back to a fresh scan of `1-SOURCES/Text/`** when the target canon's pool file has no suitable `unclaimed` entry — or when that canon is still under-scanned per `SCAN-STATUS.md` (Pali prose Nikāyas, most of the Kangyur) and you need something the pool doesn't yet hold. Whichever path, the candidate must: (a) pass the **hard gates** (§1); (b) pass **quality** (§2: a source **already short enough to translate in full** — a verse or a short prose line, *not* a summary of a long passage; prose is fine if it is short — plus **self-contained AND intelligible out of context** (a whole sentence that still needs its backstory to land — e.g. Toh 95 "this earth is my witness," or a bare "I accept your repentance" — fails §2's out-of-context test however iconic, because the card ships with no context), accessible with no glossary, relatable, and **not a bare proclamation of the Buddha's/an arahant's own attainment** — "my births are ended, the task is done, no more becoming" is authentic but reports a finished state rather than pointing the way, so it's a poor daily card, see §2), and **offers a way through, not only a bleak diagnosis** (a verse whose whole content is the trap — endless craving, "no time of release," with no remedy or relief — is too sober for a daily card even when true; prefer the parallel that carries the resolution, see §2/§3); (c) have a **theme** that isn't the previous day's (no back-to-back repeats; otherwise themes may recur freely, and filling gaps in `speaks_to`/theme coverage is a plus); and (d) be **fresh** — *not* one of the over-exposed "greatest hits" (§2 Freshness). Reach into the breadth of the corpus for under-circulated verses; the famous handful are presumed-used.
...
- [ ] Candidate is intelligible out of context (not merely grammatically whole) and accessible with no glossary — iconic-but-context-dependent lines rejected.
