...
12. **Tone/wellbeing is its own gate, distinct from buddhavacana and length.** A passage can be genuinely Buddha-spoken, self-contained, and on a good theme, and still fail — e.g. `selection-criteria.md` §3's rule that graphic or harsh imagery (torture-simile content, viscerally violent similes) stays off the daily card regardless of doctrinal soundness (found at `MĀ 5`, the wood-pile/molten-iron-balls simile). When a passage trips this gate, reject it with that reason named explicitly ("tone/wellbeing gate, not buddhavacana or length") rather than folding it into a generic "didn't fit" note — a future rotation skill may want to distinguish these categories. **A softer sibling of this gate:** a verse whose *whole* content is a bleak diagnosis (how craving/suffering traps you, "no time of release") with **no turn toward the remedy or relief** is a weak daily fit even without graphic imagery — authentic, but too sober for a once-a-day card (`selection-criteria.md` §2 "Offer a way through" / §3). You may still pool it if it's clean, but tag it `note: problem-side only — weak daily fit`, and prefer a parallel that also carries the way through. (Worked example: EĀ `ea-b`, the ordinary-person-clinging-to-forms passage — reframed from accepted to problem-side-only, 2026-07-13.)
