...
| 14 | Kangyur sūtra section, all 84000-published (Toh 8–359) | Tibetan + En | `bo-toh<N>.md` + `en-toh<N>-84000.md` | **imported** — 234 texts: PD Degé Tibetan (OpenPecha) paired with 84000's verbatim English reference (CC BY-NC-ND). Covers Prajñāpāramitā, Ratnakūṭa, General Sūtra; **no tantra/Vinaya**. **5 giant prose works trimmed** (Toh 8/9/10 Perfection of Wisdom, 44 Avataṃsaka, 287 Saddharmasmṛtyupasthāna) — not daily-verse material; moved to `0-INBOX/_trimmed-large-texts/` and excluded from the importer (audit 2026-06-24). ~229 texts remain. Pairing is file-level by Toh; block-level folio alignment (imgnum↔folio) is a refinement. |
| 15 | Dharmapada (法句經, T210) | Chinese | `zh-dharmapada.md` | **imported 2026-07** (39 品/chapters; CBETA TEI T04n0210, CC BY-NC-SA). The **Chinese verse Dharmapada** — short, Buddha-spoken gāthās, the Chinese parallel to the Pali Dhammapada. Verses segmented by CBETA line-group id (`^lg…`), chapter intros by `^p…`. Grounded via Pali Dhammapada/Udāna parallels (Sujato, CC0); own modern renderings in-vault. Gives Chinese **short full-verse** cards (the Āgamas are prose). |
| 16 | Chinese **Mahāyāna sūtras** (select) | Chinese | `zh-diamond-sutra.md`, `zh-lotus-sutra.md`, `zh-amitabha-sutra.md`, `zh-bequeathed-teachings.md`, `zh-vimalakirti.md`, `zh-eight-realizations.md` | **imported 2026-07** — 6 sūtras from CBETA (CC BY-NC-SA): Diamond (T235), Lotus (T262), Amitābha/Pure Land (T366), Bequeathed Teachings (T389), Vimalakīrti (T475), Eight Realizations of Great Beings (T779). Prose blocks anchored per-paragraph (`^p…`), verse blocks (`^lg…`). The **Chinese-language Mahāyāna voice**, complementing the Tibetan Kangyur. Own renderings in-vault; meaning cross-checked against the Tibetan/Sanskrit parallel or a standard reference. |

**Chinese canon scope boundary (what we take, what we don't).** From the Chinese canon we include the **four Āgamas** (長/中/雜/增壹阿含經 = Taishō T1, T26, T99, T125 — the discourse section, parallels to the Pali Nikāyas), the **Chinese Dharmapada** (法句經 T210), and a growing set of **Mahāyāna sūtras** (T235 Diamond, T262 Lotus, T366 Amitābha, T389 Bequeathed Teachings, T475 Vimalakīrti, T779 Eight Realizations — added 2026-07 so the Mahāyāna is represented in Chinese, not only through the Tibetan Kangyur). We **exclude** the Chinese **Vinaya (律部)** — monastic rules — the **Abhidharma / treatise sections (論部 / 毘曇)** — scholastic analysis — and the **tantric material (密教部)** (WeBuddhist serves all Buddhists; tantra is tradition-specific/restricted). This mirrors the Pali boundary (Sutta Piṭaka only) and the Kangyur boundary (mdo sūtras, no tantra/Vinaya): **across all three canons, this vault holds the Buddha's discourses and verses — sūtra, not the disciplinary, scholastic, or tantric literature.** (Mahāyāna sūtras are buddhavacana in the Mahāyāna traditions; including them is deliberate, for a "for all Buddhists" calendar.)
...
**Chinese Dharmapada (法句經 T210) — beyond the four Āgamas, on purpose.** T210 is not an Āgama; it sits in Taishō vol. 4 as a verse collection. We include it deliberately as the **Chinese counterpart to the Pali Dhammapada** — the vault holds the Buddha's *discourses and verses* across all three canons, and the Chinese Āgamas (prose) yield no short full-verse cards. T210 supplies short, Buddha-spoken Chinese gāthās that can be **quoted in full** within the app-card limit. CBETA source = CC BY-NC-SA (non-commercial, ShareAlike; cleared for our non-profit use, kept separate from CC0 Pali in output licensing).

**Grounding-by-parallel for the two verse collections (法句經 T210 · Udānavarga Toh 326).** Neither has a paired modern translation we can ship from (T210's English translations are variously licensed; 84000 has not published Toh 326). Both are, however, close **parallels to the Pali Dhammapada / Udāna**, which we hold with Sujato's CC0 English. So a rail on a T210 or Udānavarga verse establishes meaning against its **documented Pali parallel** (grounding: `parallel-pali`), and we render our own zh/bo (and the other four) from the Chinese/Tibetan source cross-checked against that parallel. Only pick verses with a clear parallel; note the parallel in the rail. This keeps the no-hallucination bar without waiting on an external translation.
...
**Ladakhi (`lbj`) note.** ISO 639-3 `lbj`; Ladakhi has no two-letter 639-1 code, and `lad` was deliberately avoided because it denotes Ladino. **Script: Tibetan (settled, Evan, 2026-09).** This field changed twice before settling, so it is recorded here to stop it being re-litigated: an initial pass assumed Tibetan script, Evan then specified Devanagari, and Evan then corrected that back to **Tibetan script**, which is final. Everything downstream (docs, termbase column, Days 76–84 `lbj` renderings) is on Tibetan script.

**The consequence, and the failure mode to guard against.** Because Ladakhi shares both its script *and* much of its written dharma vocabulary with Tibetan, an `lbj` line will often look very close to the `bo` line — and for technical terms (`སེམས`, `དྲན་པ`, `བྱམས་པ`, `ཚུལ་ཁྲིམས`) it may legitimately be identical. **That does not license copying the `bo` line across.** Ladakhi diverges from Central Tibetan in everyday vocabulary, in pronunciation-driven spelling, and in grammar — verb endings, case particles, kinship words — which is precisely the layer this vault ships in, since our renderings are modern and plain rather than liturgical. So: where the `lbj` line matches `bo`, QA must say *why* it matches; where it differs, QA must say *what* differs. An `lbj` line that is a silent duplicate of `bo` is a QA failure, not a result. It has the thinnest reviewer pool of any output language, so `lbj` always carries `escalate-native-review`. **Backfill debt:** Days 1–75 have no `lbj` rendering at all; Days 76–79 were the first.
