...
| `lbj` | **Ladakhi** (**Devanagari**) | output — **added 2026-09** |
...
**Ladakhi (`lbj`) note.** ISO 639-3 `lbj`; Ladakhi has no two-letter 639-1 code, and `lad` was deliberately avoided because it denotes Ladino. **Script: Devanagari** (Evan, 2026-09) — a product decision for WeBuddhist's Ladakhi readers; do not "correct" it to Tibetan script. Ladakhi is a **Tibetic** language written here in an Indic script, which puts it in an unusual position: its vocabulary is largely cognate with `bo`, while its script matches `hi`/`ne`. So the `lbj` rendering can never be a copy of the `bo` line (wrong script), and the failure mode to guard against is **drift into Hindi** (right script, wrong language family). QA must also record which Devanagari orthography convention for Ladakhi was assumed, since that is not settled in this vault. It has the thinnest reviewer pool of any output language, so `lbj` always carries `escalate-native-review`. **Backfill debt:** Days 1–75 have no `lbj` rendering at all; Days 76–79 were the first.
