...
- **ne** — <our rendering>
- **mn** — ⚑ <our rendering>
- **lbj** — ⚑ <our modern Ladakhi rendering, in **Devanagari** — NOT a copy of the bo line (different script) and NOT a copy of the hi line (different language)>
...
- **lbj — Ladakhi (added 2026-09), written in DEVANAGARI (Evan, 2026-09).** The seventh output language. Ladakhi is a Tibetic language, but **WeBuddhist ships it in Devanagari, not Tibetan script** — this is the product decision for our Ladakhi readers, so don't "fix" it to Tibetan script. Two consequences: (a) it is **not** a copy of the `bo` line, which is in a different script and in Central Tibetan; (b) sharing Devanagari with `hi`/`ne` makes **copying the Hindi line the live failure mode** — Ladakhi is Tibetic, not Indo-Aryan, so its words are mostly cognate with Tibetan while its script matches Hindi. Reviewer pool is the thinnest of any language here, so `lbj` always carries `escalate-native-review` and honest low-confidence flags. ISO 639-3 tag `lbj` (Ladakhi has no two-letter 639-1 code; `lad` was avoided because it means Ladino).
