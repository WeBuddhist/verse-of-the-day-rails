...
- **ne** — <our rendering>
- **mn** — ⚑ <our rendering>
- **lbj** — ⚑ <our modern Ladakhi rendering, Tibetan script — may legitimately share dharma terms with the bo line, but must NOT be a silent duplicate of it; say in QA what differs or why it matches>
...
- **lbj — Ladakhi (added 2026-09), Tibetan script (settled).** The seventh output language. **Script history, so this isn't re-litigated:** first assumed Tibetan, then changed to Devanagari, then corrected back to **Tibetan script** by Evan — final. **The failure mode to guard against:** Ladakhi shares script *and* much written dharma vocabulary with Tibetan, so an `lbj` line will often look close to `bo`, and for technical terms may legitimately be identical. That does **not** license copying `bo` across. Ladakhi diverges in everyday vocabulary, pronunciation-driven spelling, and grammar — verb endings, case particles, kinship words — which is exactly the layer we ship in. QA must state what differs, or why it matches. A silent duplicate of `bo` is a QA failure. Reviewer pool is the thinnest of any language here, so `lbj` always carries `escalate-native-review` and honest low-confidence flags. ISO 639-3 tag `lbj` (no two-letter 639-1 code exists; `lad` was avoided because it means Ladino).
