...
- **Form — short & self-contained, and intelligible on its own.** A 2–4 line verse (or one short sentence) that lands **without** needing its backstory or surrounding context. This is **two distinct tests, and a candidate must pass both**: (a) *grammatically whole* — a complete verse or one self-contained sentence (the no-distillation rule below); and (b) *understandable out of context* — a reader who does **not** know the surrounding story still grasps what the line means. A line can be perfectly whole and still fail (b): *"Evil one, this earth is my witness"* (Toh 95) is one clean sentence but is meaningless unless you already know the Māra / earth-touching scene; *"out of compassion I accept your repentance"* (DA 27) needs the confession it answers, which we have no room to supply. Such lines are **ineligible** for the daily card no matter how iconic — the card ships alone, with no context to lean on, so the quote must carry its own meaning. (Context-dependent gems can be saved for a future context-rich feature, not the standalone card.) Long, list-heavy, or syntactically dependent passages are also poor daily cards.
...
- **Must fit the app card — length is a HARD constraint, and you may not distil to fit.** The card has a fixed per-field character budget (see the recorded limit below). The *complete, undistilled* quote must fit that budget **in every one of the seven languages**. Because you may not shorten by summarising or cutting (the no-distillation rule), the only way to fit is to **choose a source that is genuinely short enough as a whole.** If the full quote does not fit, the source is **ineligible** — pick a shorter verse. No em dashes in the English.
  - **Not a cliff at exactly 125 (Evan, 2026-09).** The figure below is a *design benchmark*, not a measured hard limit; **126 characters is acceptable.** This does **not** license materially longer quotes — the gate stands — but do not discard a complete, well-chosen verse over one or two characters. Days 83 (126) and the banked Udānavarga 30 good-company verse (~126) are the reference cases.
...
- **Translatability.** Survives rendering into all seven languages (en, bo, zh, hi, ne, mn, lbj) without depending on Pali/Chinese wordplay, meter, or a pun that won't carry.
...
- **Offer a way through, not just the bind.** Prefer verses that point toward the path, a remedy, or a note of encouragement. A verse whose *entire* content is a bleak diagnosis — how craving traps you, how suffering is endless, "there is no time of release" — with no turn toward the way out is too sober for a once-a-day card, even when doctrinally true. If a passage states the problem, prefer the portion (or a parallel) that *also* carries the resolution; if only the bleak half is quotable on its own, pass it over. (This is stricter than the §3 "sober sparingly" spacing rule: even a verse you'd space out should still leave the reader with a way forward, not just the trap.)
- **Not a bare proclamation of attainment.** Skip verses whose whole content is the Buddha (or an arahant) *announcing his own accomplishment* — "my births are ended, what had to be done is done, there is no more becoming," "I am the unsurpassed one in the world," and the like. Even when these are authentic buddhavacana, self-contained, and accessible, they report a *finished state from the speaker's side* rather than teaching, guiding, or meeting the reader where they are — so they make poor daily cards for a practitioner audience. **Prefer verses that point the way** (how to practise, how to see, how to meet a hard moment) over verses that *report a result*. A teaching *about* liberation and how it is reached is fine; a bare self-declaration that one has reached it is not.
...
- **Tone — lean uplifting, sober sparingly.** The default daily tone is **encouraging or contemplative**. Sobering teachings (impermanence, mortality, the cost of unwholesome action) are part of the Dharma and **are** included — but used deliberately and occasionally, framed gently, and the harshest/graphic verses (e.g. vivid hell-realm imagery) are kept off the daily card. Rough target: a clear majority of cards uplifting/steadying, the sobering ones spaced out. **And even a sober card must leave the reader with a way through** — see §2 "Offer a way through, not just the bind." A verse that only names the trap (endless craving, no release) with no path or relief is not a daily card, however true; pick the parallel that carries the resolution instead.
...
- [ ] Short and self-contained — grammatically whole AND understandable with no backstory (iconic-but-context-dependent lines like "this earth is my witness" fail).
- [ ] Understandable by a lay reader in modern language, no glossary.
- [ ] Not a bare proclamation of the Buddha's/an arahant's own attainment — points the way, doesn't just report a result.
- [ ] Offers a way through or an uplifting turn, not only a bleak diagnosis of the problem.
...
- [ ] Translates cleanly into all seven languages (en · zh · bo · hi · ne · mn · lbj).
...
- **Toh 95 "this earth is my witness" (earth-as-witness)** — ❌ a whole, iconic sentence, but fails the **out-of-context** test (§2b): it means nothing unless you know the Māra / earth-touching scene, and the card has no room to supply it. Iconic ≠ standalone.
- **DA 27 (forgiveness — accepting Ajātaśatru's repentance)** — ❌ once the patricide/confession frame is dropped for length, "out of compassion I accept your repentance" no longer makes sense on its own. Out-of-context fail (§2b).
- **Toh 231 "the world is painted by mind, yet mind does not see mind" (mind-paints-the-world)** — ❌ whole and standalone, but too esoteric for a no-glossary lay reader — an **accessibility** fail (§2 Accessibility), a different reason from the two above.
- **Toh 95 "my births are ended, what had to be done is done, there is no further becoming" (liberation-declaration)** — ❌ authentic, whole, and clear, but a **bare proclamation of attainment** (§2): it reports the Buddha's finished state rather than teaching or speaking to the reader. Not a daily-card fit for a practitioner audience.
- **EĀ "clinging to forms → drift on through birth and death, no time of release" (ea-restraint)** — ❌ authentic and standalone, but a pure **problem-diagnosis** with no turn toward the remedy (§2 "Offer a way through") — too sober/bleak for a daily card. Prefer a sense-restraint verse that also shows the freedom guarding the senses brings.
