...
- **Length vs faithfulness — faithfulness wins.** Prefer short renderings for the app card (roughly **≤ 20 words / ≤ 120 characters in English** as a *target*), but **never cut or summarise content to hit the target.** If a genuinely short real quote runs a little long, keep it whole; if it would only fit by distilling, it fails the rule above and should not be used. No em dashes in the English. *(If the design team gives a hard per-field character limit, record it here; even then, solve length by choosing a shorter source, not by distilling.)*
